	...
	public void showQueue(CommandSender sender) {
		...
		for (String queue : Booster.getQueues()) {
			...
			if (boosters.isEmpty()) {
				...
				int timeDiffMinutes = boosters.get(0).getBooster().getDuration()
						- (int) (System.currentTimeMillis() - boosters.get(0).getStartTime()) / 1000 / 60;
				lines.add(s + ChatColor.RESET + "" + ChatColor.GREEN + boosters.get(0).getPlayer().getName() + "'s "
						+ boosters.get(0).getBooster().getName() + ", " + TimeUtils.timeFromMinutes(timeDiffMinutes)
						+ " left");
				boolean first = true;
